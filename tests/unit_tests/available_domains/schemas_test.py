...
def test_available_domains_schema_dump() -> None:
    ...
    result = schema.dump(
        {"domains": ["https://a.superset.com", "https://b.superset.com"]}
    )
    assert result["domains"] == ["https://a.superset.com", "https://b.superset.com"]


def test_available_domains_schema_dump_empty() -> None:
    schema = AvailableDomainsSchema()
    result = schema.dump({"domains": []})
    assert result["domains"] == []


def test_available_domains_schema_load() -> None:
    schema = AvailableDomainsSchema()
